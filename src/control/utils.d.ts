import { AttemptResult, BranchExec } from "../../types.js";
import type { Option } from "../../types.js";
/**
 * Functional if/else utility. Executes or returns based on a condition.
 *
 * @param condition - Boolean condition to evaluate.
 * @param value - Value passed to both branches.
 *
 * @returns An API to define the `then` and `else` branches.
 *
 * @example
 * const result = Branch(true, "hello")
 *   .then(v => `Success: ${v}`)
 *   .else(v => `Failure: ${v}`); // ➜ "Success: hello"
 *
 * @example
 * Branch(5 > 3, "hi")
 * .then((v) => console.log("True:", v))
 * .else((v) => console.log("False:", v));
 *
 */
export declare function Branch<T, R>(condition: boolean, value: T): BranchExec<T, R>;
/**
 ...
 * unless(user.isAdmin, () => console.log("Access denied"));
 */
export declare function unless(cond: boolean, fn: () => void): void;
/**
 ...
 */
export declare function repeatUntil<T>(fn: () => T, predicate: (val: T) => boolean, maxTries?: number): T;
/**
 ...
 */
export declare function doIf<T>(cond: boolean, fn: () => T): T | undefined;
/**
 ...
 */
export declare function ifValid<T, K>(value: T, fn: () => K): Option<K>;
/**
 ...
 */
export declare function attempt<T = unknown>(fn: () => T): AttemptResult<T>;

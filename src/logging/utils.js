 ...
 */
export const logTest = (message, template = "info", type = "log", color = "white", bgColor = "bgBlack") => {
    const plainMessage = String(message);
    const customStyled = () => {
        try {
            return chalk[color][bgColor](plainMessage);
        }
        catch {
            return plainMessage;
        }
    ...
    const handlers = {
        ...
        success: () => console.log(chalk.green(plainMessage)),
